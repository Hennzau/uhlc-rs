...
#[cfg(all(feature = "alloc", feature = "no-alloc"))]
compile_error!("`no-alloc` and `alloc` features cannot be enabled at the same time");

#[cfg(all(feature = "std", feature = "no-alloc"))]
compile_error!("`std` and `no-alloc` features cannot be enabled at the same time");
...
compile_error!("You must enable at least one of: `std`, `alloc`, or `no-alloc`");
...
impl HLC {
    ...
    pub fn update_with_timestamp(&self, timestamp: &Timestamp) -> Result<(), &'static str> {
        ...
        if *msg_time > now && *msg_time - now > self.delta {
            #[cfg(feature = "defmt")]
            defmt::warn!(
                "incoming timestamp exceeding delta is rejected: {} vs. now: {}",
                msg_time,
                now
            );
