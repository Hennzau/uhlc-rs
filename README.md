 ...
 * `std`: allows this crate to use the full `std`.

 * `alloc`: always enabled when `std` is enabled, allows to use the `alloc` crate
    instead of `std` when `std` is disabled. This feature enables the use of heap-allocated
    data structures like `Vec`, `String`, `Box`, etc.

 * `no-alloc`: disables the use of the `alloc` crate, hence this crate
   can be used in `no_std` environments without heap allocation support. This will
   use static friendly data structures from the `heapless` crate.
